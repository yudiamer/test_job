...
==========================
Handles sending messages and images to DingTalk
"""
...
class DingTalkSender:
    ...
    def send_chart_with_image(self, chart_image_path):
        """Send trade trends chart as actual image (not ActionCard)"""
        print("📈 Sending Trade Trends Chart as Image to DingTalk...")
        print("=" * 50)
        
        caption = f"""📊 TRADE TRENDS CHART
🕐 Time: {time.strftime('%Y-%m-%d %H:%M:%S')}

📈 Trade trends dashboard captured successfully!
📊 This chart shows current performance metrics.

---
Automated monitoring system"""
        
        success = self.send_image_message(chart_image_path, caption)
        
        print("=" * 50)
        if success:
            print("🎉 Trade Trends Chart sent as image successfully!")
        else:
            print("❌ Failed to send Trade Trends Chart as image")
